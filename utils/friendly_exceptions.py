...
def msg(e):
    """ This function returns some explanations to typical
        exceptions encountered by beginners.
         Hints on how to remove the root casue are given"""

    if type(e).__name__ == "NameError":
        return "Es wurde eine Exception vom Type {} ausgelöst.\nStellen Sie sicher, dass die Namen der von Ihnen verwendeten Funktionen korrekt sind.\nEvtl. müssen Sie die Zelle, in der Ihre Funktion definert ist, neu ausführen.".format(
            type(e).__name__)

    elif type(e).__name__ == "UnboundLocalError":
        return "Es wurde eine Exception vom Type {} ausgelöst.\nStellen Sie sicher, dass alle Variable vor ihrer Verwendung korrekt initialisiert werden.".format(
            type(e).__name__)
